...
-(id) init {
	if((self=[super init])) {
		...
		CCSprite *background = [CCSprite spriteWithFile:@"game-complete-scene.png"];
		background.position = ccp(s.width, s.height/2);
		[self addChild:background z:-10];
		[self schedule:@selector(updateCamera:)];
		
		[[ProgressManager getOrInitProgress] playGameCompleteTheme];
	}
	return self;
}
...
-(void) ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event {
	[[CCDirector sharedDirector] replaceScene: [CCTransitionFade transitionWithDuration:1 scene:[MainScene scene]]];
}
