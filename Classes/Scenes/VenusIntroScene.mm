...
@implementation VenusIntroScene

+(id) scene {
	CCScene *scene = [CCScene node];
	id child = [VenusIntroScene node];
...
-(id) init {
	if((self=[super init])) {
		...
		CCSprite *background = [CCSprite spriteWithFile:@"venus-intro-scene.png"];
...
-(void) updateCamera:(ccTime)dt {
	...
	if (self.position.x < -480) {
		introEndDelay_ -= dt;
		if (introEndDelay_ <= 0) {
			[[CCDirector sharedDirector] replaceScene: [CCTransitionFade transitionWithDuration:1 scene:[Level20 scene]]];
		}
