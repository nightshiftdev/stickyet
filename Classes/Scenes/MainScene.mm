...
-(id) init {
	if((self=[super init])) {
		...
		CCMenuItem *itemPlay = [SoundMenuItem itemFromNormalSpriteFrameName:@"btn-play-normal.png" selectedSpriteFrameName:@"btn-play-selected.png" target:self selector:@selector(selectEpisode:)];
		CCMenu *menuPlay = [CCMenu menuWithItems: itemPlay, nil];
		menuPlay.position = ccp(s.width/2 - 15, s.height/2 - 80);
...
-(void) showCredits:(id)sender {
	[[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:1 scene:[CreditsScene scene]]];
}
...
-(void) howToPlayCallback:(id)sender {
	[[CCDirector sharedDirector] replaceScene:[CCTransitionCrossFade transitionWithDuration:1 scene:[HowToPlayScene scene:YES]]];
}
