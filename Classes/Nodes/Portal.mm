...
-(id) initWithBody:(b2Body*)body game:(GameNode*)game
{
	if( (self=[super initWithBody:body game:game]) ) {
		...
		b2CircleShape	shape;
		shape.m_radius = 1.0f;		// 0.5 meter of diameter
		b2FixtureDef	fd;
		fd.shape = &shape;
		...
		// default teleport to
		teleportTo_ = CGPointMake(-100.0, -100.0);//CGPointZero;
		
		// rotate portal
		id rot = [CCRotateBy actionWithDuration:4 angle:360];
...
-(void) touchedByHero
{
	...
	[game_ gameOver:YES touchedFatalObject:NO];
}
